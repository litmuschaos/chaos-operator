...
func CheckDeploymentAnnotation(clientSet *kubernetes.Clientset, engine types.EngineInfo) error {
	...
	for _, app := range targetAppList.Items {
		...
		annotationValue := app.ObjectMeta.GetAnnotations()[types.ChaosAnnotationKey]
		chaosCandidates, err = ValidateAnnotation(annotationValue, chaosCandidates)
		if err != nil {
			return err
		}
		types.Log.Info("chaos candidate : ", "appName", engine.AppName, "appUUID", engine.AppUUID)
	}
	return nil
}
