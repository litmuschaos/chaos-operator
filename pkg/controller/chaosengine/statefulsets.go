...
func checkAnnotationStatefulSet(clientSet *kubernetes.Clientset) error {

	targetApplicationStatefulSetList, err := clientSet.AppsV1().StatefulSets(engine.appInfo.namespace).List(metav1.ListOptions{LabelSelector: engine.instance.Spec.Appinfo.Applabel, FieldSelector: ""})
	if err != nil {
		log.Error(err, "unable to list apps matching labels")
		return err
	}
	chaosCandidates := 0
	if len(targetApplicationStatefulSetList.Items) == 0 {
		return errors.New("No app statefulset with matching labels")
	}
	for _, app := range targetApplicationStatefulSetList.Items {
